						...
						{data.length > 0 &&
							data.map((item, index) => (
								<div
									...
									key={index}
									className="flex justify-between items-center border-b py-3 cursor-pointer hover:shadow-md px-2 "
								>
									<div className="flex">
										<img
										...
										<div className="flex flex-col px-2 w-full">
											<span
												className={`text-sm ${
													playing?._id === item?._id ? 'text-green-500' : 'text-red-500 '
												} capitalize font-semibold pt-1`}
											>
												{item.name}
											</span>
											<span className="text-xs text-gray-500 uppercase font-medium ">- {item.author}</span>
										</div>
									</div>
									<button
										onClick={(e) => {
									...
									>
										<svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
											<g clipPath="url(#clip0_632_10823)">
												<path
